...
description: |
  Niche skill for AI Hero / ai-hero-cli course-authoring repos only. Creates exercise directory structures - sections (`XX-section-name/`), exercises (`XX.YY-exercise-name/`), and per-exercise variant subfolders (`problem/`, `solution/`, `explainer/`, `explainer.1/`) with stub `readme.md` (and optional `main.ts`) files that pass the `pnpm ai-hero-cli internal lint` validator. Use ONLY when working inside a Matt Pocock / AI Hero course repo or fork (e.g., `ai-hero-dev/ai-hero`, `ai-sdk-v6-crash-course`, cohort project repos) where the linter expects this exact convention. Trigger when the user says: "scaffold exercises for section X", "create exercise stubs", "set up a new course section", "add exercises 03.04-03.07 to the retrieval section", "stub out the memory section", "create problem/solution/explainer folders", "renumber exercise X.Y to X.Z", "add explainer.1 variant", "make exercise stubs that pass the lint", or "scaffold the next cohort module". Skip when the project does NOT have `ai-hero-cli` as a dependency, when the user wants generic project scaffolding (use the framework's official generator, e.g., `pnpm create next-app`, `npx create-video`, `pnpm create vite`), when the user wants exercise/test stubs for a non-AI-Hero course, when they're authoring tutorials in a different format (Egghead, MDX-only blogs, etc.), or when the task is to write actual exercise content rather than directory structure. Requires `pnpm` and the repo's `ai-hero-cli` dev dependency installed (`pnpm install` first). Final step is always `pnpm ai-hero-cli internal lint` - the scaffold is not "done" until that passes.
metadata:
  tags: ai-hero, course-authoring, scaffolding, exercises, ai-hero-cli, niche
  requires-cli: true
