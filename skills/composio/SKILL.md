---
name: composio
description: Connect AI agents to 1000+ external SaaS apps via Composio's tool router. Managed OAuth, API-key vaults, automatic token refresh — agent never sees raw credentials. Covers Slack, Discord, MS Teams, Gmail, Outlook, GitHub, GitLab, Linear, Jira, Notion, Asana, Trello, HubSpot, Salesforce, Pipedrive, Airtable, Google Sheets, Drive, Dropbox, Calendar, Mailchimp, Twilio, and 850+ more. Use when the user says "send Slack message", "send Gmail", "create GitHub issue", "open PR", "create Linear ticket", "update Notion page", "add Airtable record", "fetch HubSpot contact", "update Salesforce lead", "create calendar event", "upload to Drive", "send WhatsApp/SMS", "automate cross-app workflow", or any "do X in <SaaS app>" request. Skip when a dedicated MCP for that app is already attached. Requires the Composio CLI installed (`composio login`).
