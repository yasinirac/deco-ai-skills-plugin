...
description: |
  Domain-specific best practices for Remotion - the React framework for programmatic, code-driven video. Covers compositions, sequencing, frame-based animation with `useCurrentFrame()` + `interpolate()` + `spring()`, asset handling (Img/Video/Audio + `staticFile`), dynamic metadata via `calculateMetadata`, captions/subtitles, FFmpeg trims, silence detection, audio visualization (waveforms, spectrum bars, bass-reactive), SFX, 3D with Three.js / React Three Fiber, light leaks, Lottie, GIFs, HTML-in-canvas, measuring DOM nodes/text, transitions, transparent renders, parameterized videos with Zod, AI voiceover via ElevenLabs, fonts (Google Fonts + local), mapbox flyovers, and Remotion Studio + render workflows. Use when the user says any of: "create a video", "make a video in React", "generate a video programmatically", "Remotion composition", "Remotion render", "useCurrentFrame", "useVideoConfig", "interpolate animation", "Sequence component", "Composition Root.tsx", "data-driven video", "personalized video", "automated video pipeline", "animated logo", "animated explainer", "kinetic typography", "lower-thirds", "title cards", "video with captions / subtitles", "burn-in subtitles", "video with voiceover", "TTS narrated video", "video with waveform", "social media reel from data", "LinkedIn / Instagram / TikTok video", "MP4 from React", "render video CLI", "transparent background video", "data viz video", "scroll-style video", "scene transition", "video parameters / Zod schema", "video duration calculate", or any task involving programmatic video creation in JavaScript/TypeScript with React. Use any time you are reading or writing code under a project that has `remotion`, `@remotion/cli`, or `@remotion/media` in its dependencies, or any folder containing `src/Root.tsx` with a `<Composition>` registration. Skip when the user wants a non-programmatic timeline editor (recommend CapCut, Premiere, DaVinci, Adobe Express), when the task is just trimming/concatenating files (use FFmpeg directly via the `video` skill), when generating static images (use the `image` skill), or when designing the storyboard / brief itself before any code (use the `video` or `frontend-design` skills first). Requires Node 18+, the `remotion` package, and FFmpeg on PATH for renders that need re-encoding. Forbids CSS transitions, CSS keyframe animations, and Tailwind animation classes - they will not render correctly; all motion MUST come from `useCurrentFrame()` + `interpolate` / `spring`.
metadata:
  tags: remotion, video, react, animation, composition, programmatic-video, captions, ffmpeg, voiceover, three-js, lottie, transitions
  requires-cli: true
