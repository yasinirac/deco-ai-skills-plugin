...
description: Best practices for Remotion - the React framework for programmatic, code-driven video. Covers compositions, frame-based animation with `useCurrentFrame()` + `interpolate()` + `spring()`, captions, FFmpeg, audio waveforms, 3D, transitions, parameterized videos with Zod, AI voiceover. Use when the user says "create a video in React", "make a Remotion composition", "useCurrentFrame", "Sequence component", "data-driven video", "personalized video", "animated logo", "animated explainer", "kinetic typography", "video with captions/subtitles", "video with voiceover", "social media reel from data", "MP4 from React", "render video CLI", "transparent video", or works in any project with `remotion` in dependencies. Skip when user wants a timeline editor (recommend CapCut/Premiere) or just trimming files (use FFmpeg). Requires Node 18+ and FFmpeg. **Forbids CSS animations and Tailwind animation classes** — all motion must use `useCurrentFrame()` + `interpolate`/`spring`.
metadata:
  tags: remotion, video, react, animation, composition, programmatic-video, captions, ffmpeg, voiceover
  requires-cli: true
